...
const NewDeathsGraph = ({ countryName, data }: MyProps) => {
  return (
    <div className="w-full h-full flex items-center justify-center flex-col gap-3">
      <div className="flex items-center justify-center flex-col text-center">
        <h2 className="text-xl md:text-2xl">Daily new deaths in {countryName}</h2>
      ...
      <ResponsiveContainer width={screen.width < 768 ? '95%' : 800} height={400}>
        ...
        >
          ...
          <Bar dataKey="NewDeaths" fill="#353b48" />
        </BarChart>
      </ResponsiveContainer>
    </div>
  );
